...
console.log('🔍 Debug article-detail.js:');
console.log('- URL actuelle:', window.location.href);
console.log('- Pathname:', window.location.pathname);
console.log('- Dernier élément du path:', lastPathElement);
console.log('- ID depuis paramètres:', articleId);
...
async function loadArticleById(id) {
    console.log('📄 Chargement article par ID:', id);
    currentArticleId = id;
    ...
    try {
        ...
        if (!docSnapshot.exists()) {
            console.error('❌ Document non trouvé pour ID:', id);
            showError();
            return;
        }
        
        console.log('✅ Article trouvé:', docSnapshot.id);
        const article = docSnapshot.data();
        console.log('📝 Données article:', article.title);
        ...
        // Charger les données supplémentaires
        console.log('⏳ Chargement des données supplémentaires...');
        await Promise.all([
            loadReactions(id),
            loadComments(id),
            loadRelatedArticles(article.category)
...
function showError() {
    console.error('❌ Affichage de l\'erreur - Article non trouvé');
